...
pub fn add_comment(ctx: Context<AddCommentContext>, comment_content: String) -> Result<()> {
    ...
    // -------------------------------------------------------------------------------------------
    // Check the length of the comment_content to ensure it doesn't exceed the maximum allowed length.
    require!(
    ...
    // -------------------------------------------------------------------------------------------
    // Copy the comment_content into the comment's bytearray
    let mut content_data = [0u8; 500];
    content_data[..comment_content.as_bytes().len()].copy_from_slice(comment_content.as_bytes());
    comment.content = content_data;

    // -------------------------------------------------------------------------------------------
    // Set other fields in the Comment account
